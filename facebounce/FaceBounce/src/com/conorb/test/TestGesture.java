...
public class TestGesture {
	...
	Hmm<ObservationVector> learntHmmSend=null;
	 String root = Environment.getExternalStorageDirectory().toString();
	    File myDir = new File(root + "/Data1");    
    ...
    public String test(File seqfilename) throws Exception{
        ...
        for (int i = 0; i < testSequences.size(); i++) {
           ...
           // gesture = 1;
            ...
            if ( punchProbability > scrolldownProbability && punchProbability > sendProbability) {
                gesture = 1;
            }
            
            if (scrolldownProbability > punchProbability && scrolldownProbability > sendProbability) {
                gesture = 2;
            }
            
            
           if (sendProbability > punchProbability && sendProbability > scrolldownProbability)
           {
        	   gesture = 3;
           }
            ...
            if (gesture == 1) {
            	System.out.println("This is an stomp gesture");
            	return "stomp";
            } else if (gesture == 2) {
